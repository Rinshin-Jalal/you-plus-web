...
class IntelligenceService {
  static build(identity: Identity | null, userContext: UserContext): string {
    if (!identity) return "**WARNING**: No identity data available.\n";
    ...
    if (context.goal) intel += `**Goal**: "${context.goal}"\n`;
    if (context.motivation_level) intel += `**Motivation**: ${context.motivation_level}/10\n`;
    if (context.favorite_excuse) intel += `**Favorite Excuse**: "${context.favorite_excuse}"\n`;
    if (context.future_if_no_change) intel += `**Future If No Change**: "${context.future_if_no_change}"\n`;
    if (context.who_disappointed) intel += `**Who Disappointed**: "${context.who_disappointed}"\n`;
    if (context.quit_pattern) intel += `**Quit Pattern**: "${context.quit_pattern}"\n`;
