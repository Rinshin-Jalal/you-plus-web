...
export class UserScheduleEngine {
   ...
   *
   * 🚧 CONSTRAINTS:
   *    • Max 10 concurrent calls (telephony service limits)
  ...
  private async batchProcessCalls(
    ...
    // ⚡ Process in chunks of 10 to respect telephony concurrent call limits
    const BATCH_SIZE = 10;
   ...
   * Uses user's timezone to determine next available call slot
   */
  private calculateNextCallTime(
    ...
    try {
      ...
      // Default call window: 8 AM in user's timezone
      const defaultCallHour = 8;
      const defaultCallMinute = 0;

      // Check if we're already past today's window
      const todayWindowStart = new Date(userLocalTime);
      todayWindowStart.setHours(defaultCallHour, defaultCallMinute, 0, 0);
