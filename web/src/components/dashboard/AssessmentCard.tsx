...
export const AssessmentCard = ({ status, trustScore, hasCompletedFirstCall = true }: AssessmentCardProps) => {
  ...
  if (!hasCompletedFirstCall) {
    return (
      <div className="bg-white/5 border border-dashed border-white/20 p-5 md:p-6 relative overflow-hidden">
        <div className="relative z-10">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              ...
              <h3 className="font-bold text-xl uppercase tracking-tight text-white/50">
                AWAITING DATA
              </h3>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-2">
            ...
            <div className="h-2.5 w-full bg-white/10 overflow-hidden">
  ...
  return (
    <div className={`${config.bg} border-2 ${config.border} p-5 md:p-6 relative overflow-hidden`}>
      
      {/* Background Score Watermark */}
      <div className="absolute -bottom-6 -right-2 text-[7rem] font-bold opacity-[0.07] text-white pointer-events-none leading-none select-none">
        {trustScore}
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="font-bold text-xl uppercase tracking-tight text-white">
                {config.label}
              </h3>
            </div>
          </div>
        ...
        {/* Message */}
        <p className="font-mono text-xs leading-relaxed text-white/60 mb-5">
          {config.message}
        </p>
        ...
        {status && (
          <div className="mt-4 pt-4 border-t border-white/10 flex gap-4">
            <div className="flex-1">
              <p className="font-mono text-[9px] uppercase tracking-widest text-white/40">Total Calls</p>
              <p className="font-bold text-lg text-white">{status.total_calls_completed}</p>
            </div>
            <div className="flex-1">
              <p className="font-mono text-[9px] uppercase tracking-widest text-white/40">Best Streak</p>
              <p className="font-bold text-lg text-white">{status.longest_streak_days}d</p>
            </div>
          </div>
        )}
      </div>
