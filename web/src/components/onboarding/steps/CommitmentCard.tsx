  ...
  return (
      <div className="w-full max-w-lg animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="text-center mb-10">
              <div className="inline-flex items-center justify-center w-16 h-16 border-2 border-orange-500 rounded-md mb-6">
          ...
          {/* User Info Summary */}
          <div className="bg-white/5 border border-orange-500/30 rounded-md p-6 mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedPillars.map((pillarId: string) => {
                  ...
                  return (
                    <span 
                      key={pillarId}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-500/20 text-orange-400 border border-orange-500/30 rounded font-mono text-sm"
                    >
                      <span>{icon}</span>
                      <span>{label}</span>
                    </span>
          ...
          <div className="space-y-3 mb-8">
              {commitments.map((commitment, i) => (
                  <div 
                      key={i} 
                      className="flex items-start gap-4 p-4 border border-white/10 rounded-md bg-white/5"
                      style={{ animationDelay: `${i * 100}ms` }}
                  >
                      <div className="w-6 h-6 rounded-sm border-2 border-orange-500/30 flex items-center justify-center flex-shrink-0 mt-0.5">
          ...
          {/* Agreement Checkbox */}
          <label className="flex items-start gap-4 p-5 border-2 border-white/10 rounded-md cursor-pointer hover:border-orange-500/50 transition-colors mb-8">
              <input 
                  type="checkbox" 
                  checked={agreed}
