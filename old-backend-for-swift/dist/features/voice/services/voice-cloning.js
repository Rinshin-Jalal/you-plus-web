...
export class VoiceCloneService {
     ...
     * ═══════════════════════════════════════════════════════════════════════════════ */
    async cloneWithCartesia(request) {
        try {
     ...
     * ═══════════════════════════════════════════════════════════════════════════════ */
    async cloneWithElevenLabs(request) {
        try {
            console.log(`🎤 Initiating voice clone pipeline for user ${request.user_id}...`);
            ...
            formData.append("description", `YOU+ AI voice clone for user ${request.user_id}`);
            formData.append("files", new Blob([audioBuffer], { type: "audio/mpeg" }), "voice_sample.mp3" // 📄 Audio file for cloning
            );
            ...
            const result = await response.json();
            console.log(`✅ Voice clone successful! Voice ID: ${result.voice_id} for user ${request.user_id}`);
            ...
            console.error(`💥 Voice cloning pipeline failed for user ${request.user_id}:`, error);
 ...
 */
export function createVoiceCloneService(env) {
 ...
 */
export async function cloneVoice(request, env) {
