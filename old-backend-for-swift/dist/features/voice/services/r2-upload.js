 ...
 */
export async function uploadAudioToR2(env, audioBuffer, fileName, contentType = "audio/m4a") {
 ...
 * @returns Promise with upload result
 */
async function uploadToR2(env, fileBuffer, fileName, contentType, folder) {
    try {
        const fileType = folder === "audio" ? "🎵" : "🖼️";
        console.log(`${fileType} Uploading ${folder} file: ${fileName} (${fileBuffer.byteLength} bytes)`);
 ...
 */
export function generateAudioFileName(userId, recordingId, extension = "m4a") {
 ...
 */
export function extractFileExtension(uri) {
