...
export const processedEvents = new Map();
 ...
 */
export const postRevenueCatWebhook = async (c) => {
    const env = c.env;
    ...
    try {
        ...
        if (lastProcessed && now - lastProcessed < RATE_LIMIT_WINDOW) {
            ...
            console.log(`   Processed: ${Math.round((now - lastProcessed) / 1000)}s ago`);
            console.log(`   Rate limit: ${RATE_LIMIT_WINDOW / 1000}s window`);
            console.log(`   Cache size: ${processedEvents.size} events`);
            console.log(`🚫 Event blocked - preventing duplicate subscription update`);
            return c.json({
                success: true,
        ...
        console.log(`Processing RevenueCat webhook: ${type} for user ${app_user_id} (${processedEvents.size} events in cache)`);
        ...
        console.log(`  - Price: ${event.price_in_purchased_currency || "N/A"} ${event.currency || ""}`);
        ...
        switch (type) {
            ...
            case "RENEWAL":
                ...
                break;
            case "UNCANCELLATION":
                console.log(`♻️ SUBSCRIPTION UNCANCELLED - Setting subscription to ACTIVE`);
                subscription_status = "active";
                break;
            case "CANCELLATION":
                console.log(`❌ SUBSCRIPTION CANCELLED - Setting subscription to CANCELLED`);
