 ...
 */
export const postElevenLabsWebhook = async (c) => {
    const env = c.env;
    try {
        ...
        if (webhookSecret && signature) {
            ...
            const isValidSignature = webhookHandler.validateSignature(rawBody, signature, webhookSecret);
            if (!isValidSignature) {
 ...
 */
export const postElevenLabsAudioWebhook = async (c) => {
    const env = c.env;
    try {
 ...
 */
export const getElevenLabsWebhookTest = async (c) => {
    return c.json({
