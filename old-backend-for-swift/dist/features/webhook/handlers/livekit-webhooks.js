 ...
 */
export const postLiveKitWebhook = async (c) => {
    const env = c.env;
    try {
        ...
        const webhookSecret = env.LIVEKIT_WEBHOOK_SECRET || undefined;
        ...
        if (webhookSecret && signature) {
            ...
            const isValidSignature = processor.validateSignature(rawBody, signature, webhookSecret);
            if (!isValidSignature) {
        ...
        switch (webhookEvent.event) {
            ...
            case "recording_finished":
                if (webhookEvent.recording) {
                    await processor.handleRecordingFinished(env, webhookEvent.recording);
                }
                break;
            case "participant_joined":
                if (webhookEvent.participant) {
                    await processor.handleParticipantJoined(env, webhookEvent.participant);
                }
                break;
 ...
 */
export const getLiveKitWebhookTest = async (c) => {
    const env = c.env;
    try {
        const testEvent = {
            event: "room_finished",
        ...
        const processor = createLiveKitWebhookProcessor({
            webhookSecret: env.LIVEKIT_WEBHOOK_SECRET || undefined,
        });
 ...
 */
export const getLiveKitWebhookHealth = async (c) => {
    const env = c.env;
    const hasCredentials = !!env.LIVEKIT_API_KEY &&
