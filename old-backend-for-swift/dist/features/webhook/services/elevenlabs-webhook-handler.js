...
export class ElevenLabsWebhookHandler {
    ...
    async processWebhook(event) {
        try {
            console.log(`🔔 Processing ElevenLabs webhook: ${event.type} for conversation ${event.data?.conversation_id || "unknown"}`);
            switch (event.type) {
                case "post_call_transcription":
                    ...
                    return await this.handleTranscriptionWebhook(event.data);
                case "post_call_audio":
                    if (!event.data) {
                        return { success: false, error: "Missing webhook data for audio" };
                    }
                    return await this.handleAudioWebhook(event.data);
                default:
    ...
    async handleTranscriptionWebhook(data) {
        ...
        try {
            ...
            const callTypeCandidate = dynamicVars.callType ||
            ...
            const resolvedCallType = validCallTypes.includes(String(callTypeCandidate))
            ...
            const callRecord = {
                ...
                start_time: new Date(data.metadata.start_time_unix_secs * 1000)
                    .toISOString(),
                end_time: new Date((data.metadata.start_time_unix_secs +
                    data.metadata.call_duration_secs) * 1000).toISOString(),
            ...
            if (callRecordingId) {
                ...
                await this.processEvaluationResults(callRecordingId, data.conversation_id, data.analysis.evaluation_criteria_results, supabase);
                // Process data collection results
                await this.processDataCollection(callRecordingId, data.conversation_id, data.analysis.data_collection_results, userId, supabase);
            }
            ...
            console.log(`✅ Successfully processed transcription webhook for conversation: ${data.conversation_id}`);
            return { success: true };
        }
        catch (error) {
    ...
    async handleAudioWebhook(data) {
        ...
        try {
            ...
            const fileName = generateAudioFileName("elevenlabs", data.conversation_id, "mp3");
            ...
            const r2Upload = await uploadAudioToR2(this.env, arrayBuffer, fileName, "audio/mpeg");
            ...
            if (audioUrl) {
                ...
                if (updateError) {
                    console.error("Failed to update calls with audio URL:", updateError);
                }
            }
            ...
            if (r2Upload.success) {
                ...
                console.warn(`⚠️ R2 upload failed, stored base64 in database: ${r2Upload.error}`);
            }
            console.log(`✅ Successfully processed audio webhook for conversation: ${data.conversation_id} (${fileSizeBytes} bytes)`);
            return { success: true };
        }
        catch (error) {
     ...
     */
    async processEvaluationResults(callRecordingId, conversationId, evaluationResults, supabase) {
        try {
            // Store individual evaluation results for detailed analysis
            const evaluationRecords = Object.entries(evaluationResults).map(([criteriaId, result]) => ({
            ...
            if (evaluationRecords.length > 0) {
                ...
                if (error) {
                    ...
                    console.log(`✅ Stored ${evaluationRecords.length} evaluation results`);
                }
            }
        }
        catch (error) {
     ...
     */
    async processDataCollection(callRecordingId, conversationId, dataCollectionResults, userId, supabase) {
        try {
            // Store individual data collection results
            const dataRecords = Object.entries(dataCollectionResults).map(([fieldId, value]) => ({
                call_recording_id: callRecordingId,
                conversation_id: conversationId,
                field_id: fieldId,
            ...
            if (dataRecords.length > 0) {
                ...
                if (error) {
                    ...
                    console.log(`✅ Stored ${dataRecords.length} data collection results`);
                }
            }
            // Process specific data types for business logic
            await this.processBusinessData(dataCollectionResults, userId, conversationId, supabase);
            // Process promise-related data from calls
            await this.processPromiseData(dataCollectionResults, userId, conversationId, supabase);
        }
        catch (error) {
     ...
     */
    async processBusinessData(dataResults, userId, conversationId, supabase) {
        try {
     ...
     */
    async triggerFollowUpActions(callRecord) {
        try {
            // Example: Send follow-up email if call was unsuccessful
            if (callRecord.call_successful === "failure" && callRecord.user_id) {
                console.log(`📧 Triggering follow-up for unsuccessful call: ${callRecord.conversation_id}`);
            ...
            if (callRecord.evaluation_results) {
                ...
                if (positiveResults > 0 && callRecord.user_id) {
                    console.log(`✨ Positive call outcome for user: ${callRecord.user_id}`);
                    // Update user engagement metrics
                }
            }
            // Example: Trigger CRM integration
            if (callRecord.data_collection_results &&
                Object.keys(callRecord.data_collection_results).length > 0) {
                console.log(`🔄 Triggering CRM update for conversation: ${callRecord.conversation_id}`);
     ...
     */
    async processPromiseData(dataResults, userId, conversationId, supabase) {
        if (!userId)
            return;
        try {
            console.log(`🎯 Processing promise data for user ${userId} from call ${conversationId}`);
            // Process promises made during the call
            if (dataResults.promises_made || dataResults.new_promises ||
            ...
            if (dataResults.promise_updates || dataResults.promise_completions ||
                ...
                await this.updatePromisesFromCall(updates, userId, conversationId, supabase);
            }
            // Process excuses given for broken promises
            if (dataResults.excuses_given || dataResults.excuses_provided ||
                ...
                await this.processCallExcuses(excuses, userId, conversationId, supabase);
            }
            // Process accountability metrics and insights
            if (dataResults.accountability_score || dataResults.motivation_level ||
                dataResults.commitment_quality) {
                await this.updateAccountabilityMetrics(dataResults, userId, conversationId, supabase);
            }
            // Process psychological insights extracted during call
            if (dataResults.psychological_insights || dataResults.behavior_patterns ||
     ...
     */
    async saveCallPromises(promisesData, userId, conversationId, supabase) {
        try {
            ...
            for (const promise of promises) {
                if (typeof promise === "string" && promise.trim()) {
                    ...
                    console.log(`✅ Saved promise from call: "${promise.substring(0, 50)}..."`);
     ...
     */
    async updatePromisesFromCall(updatesData, userId, conversationId, supabase) {
        try {
     ...
     */
    async processCallExcuses(excusesData, userId, conversationId, supabase) {
        try {
            ...
            for (const excuse of excuses) {
                if (typeof excuse === "string" && excuse.trim()) {
                    ...
                    const relatedPromise = await this.findRelatedPromise(excuseText, userId, supabase);
                    ...
                    console.log(`🔍 Processed excuse: "${excuseText.substring(0, 50)}..."`);
                }
            }
        }
        catch (error) {
     ...
     */
    async updateAccountabilityMetrics(dataResults, userId, conversationId, supabase) {
        try {
            const metrics = {
     ...
     */
    async updatePsychologicalProfile(dataResults, userId, supabase) {
        try {
    ...
    inferPromisePriority(promiseText) {
        const urgentWords = [
            "urgent",
            "critical",
            "important",
    ...
    async categorizePromise(promiseText) {
    ...
    detectTimeSpecific(promiseText) {
        const timePatterns = [
        ...
        return timePatterns.some((pattern) => pattern.test(promiseText.toLowerCase()));
    }
    extractTargetTime(promiseText) {
        const timeMatch = promiseText.match(/(\d{1,2}:\d{2}|\d{1,2}(am|pm))/i);
        return timeMatch ? timeMatch[0] : null;
    }
    normalizePromiseStatus(status) {
        const lowerStatus = status.toLowerCase();
        if (lowerStatus.includes("complete") || lowerStatus.includes("done") ||
            lowerStatus.includes("kept")) {
            return "kept";
        }
        if (lowerStatus.includes("broken") || lowerStatus.includes("failed") ||
            lowerStatus.includes("missed")) {
            return "broken";
        }
        return "pending";
    }
    async findRelatedPromise(excuseText, userId, supabase) {
        try {
            ...
            const { data: promises } = await supabase
                ...
                .eq("user_id", userId)
                .eq("status", "pending")
                .gte("promise_date", new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split("T")[0])
 ...
 */
export function createElevenLabsWebhookHandler(env) {
