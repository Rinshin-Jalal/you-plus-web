...
export class RevenueCatService {
    ...
    constructor(env) {
        ...
        if (!apiKey) {
            console.warn("⚠️ REVENUECAT_API_KEY not set - subscription validation disabled");
            this.apiKey = "";
        }
        else {
            this.apiKey = apiKey;
    ...
    async hasActiveSubscription(userId) {
        ...
        if (isDevelopment || !this.apiKey) {
            console.warn(`🔓 ${isDevelopment ? 'DEVELOPMENT MODE' : 'API KEY MISSING'}: Bypassing subscription check for user ${userId}`);
            return {
        ...
        try {
            ...
            const now = new Date();
            const activeEntitlements = Object.values(entitlements).filter((entitlement) => {
 ...
 */
export function createRevenueCatService(env) {
