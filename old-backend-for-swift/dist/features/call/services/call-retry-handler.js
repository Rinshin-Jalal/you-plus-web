 ...
 */
export async function handleMissedCall(userId, callType, callUUID, reason, env) {
    ...
    const recentFailures = userContext.recentStreakPattern?.filter((p) => p.status === "broken");
    console.log("userContext", userContext);
    // Send to retry call
    await sendVoipPushNotification(userContext.user.push_token || "", {
        userId,
        callType,
    ...
    console.log(`⏰ Retry ${retryAttemptNumber} scheduled for ${timeoutAt.toISOString()}`);
}
/**
 ...
 */
export async function clearCallRetries(userId, callType, env) {
    const supabase = createSupabaseClient(env);
 ...
 */
export async function getRetryStatus(userId, callType, env) {
    const supabase = createSupabaseClient(env);
 ...
 */
function getEscalatedUrgency(attemptNumber) {
    if (attemptNumber === 1)
        return "high";
    if (attemptNumber === 2)
        return "critical";
 ...
 */
function getEscalatedMessage(attemptNumber, userContext) {
    const baseMessages = {
