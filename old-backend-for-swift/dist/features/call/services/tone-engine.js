...
/** Public entry point */
export function calculateOptimalTone(userContext, config = DEFAULT_SIMPLE_CONFIG) {
    ...
    const toneScore = successRate * 0.5 + // success matters most
        streakScore * 0.3 +
        (100 - collapseScore) * 0.2; // lower collapse => better tone
    // 5️⃣ Choose tone based on thresholds
    if (toneScore >= config.encouragementThreshold)
        return "Encouraging";
    if (toneScore <= config.interventionThreshold)
...
function computeSuccessRate(promises) {
...
export function generateBigBruhIdentity(identity, tone) {
