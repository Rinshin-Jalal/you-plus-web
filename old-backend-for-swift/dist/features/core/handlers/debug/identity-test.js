 ...
 */
export async function postTestIdentityExtraction(c) {
    const env = c.env;
    ...
    try {
        ...
        const extractionResult = await extractAndSaveV3Identity(userId, "TestUser", mockResponses, env);
        return c.json({
            success: true,
            mockLevel,
            extractionResult,
 ...
 */
function createMockOnboardingResponses(level) {
    const mockResponses = {};
 ...
 */
export async function deleteTestIdentityData(c) {
    const env = c.env;
