 ...
 */
async function transcribeAudioWithDeepgram(env, audioBuffer) {
    try {
 ...
 */
function generateFileName(stepId, fileType, extension) {
 ...
 */
async function processAudioResponse(env, userId, stepId, response) {
    try {
        ...
        if (!transcription.trim()) {
            console.warn(`⚠️ Transcription failed for step ${stepId} - proceeding with upload anyway`);
        }
        ...
        const uploadResult = await uploadAudioToR2(env, audioBuffer, fileName, parsed.mimeType);
        ...
        console.log(`✅ Audio transcribed and uploaded: "${transcription?.substring(0, 50)}..." → ${uploadResult.cloudUrl}`);
 ...
 */
export async function processOnboardingFiles(env, responses, userId) {
    try {
        ...
        for (const [stepId, response] of Object.entries(responses)) {
            ...
            if (response.type === "voice") {
                ...
                const result = await processAudioResponse(env, userId, parseInt(stepId), response);
        ...
        console.log(`- Processed responses: ${Object.keys(processedResponses).length}`);
 ...
 */
export function validateFileData(dataUrl, maxSize = 10 * 1024 * 1024) {
    try {
        if (!dataUrl || !dataUrl.startsWith("data:")) {
        ...
        if (estimatedSize > maxSize) {
            return {
                valid: false,
                error: `File too large: ${Math.round(estimatedSize / 1024 / 1024)}MB (max ${Math.round(maxSize / 1024 / 1024)}MB)`,
                size: estimatedSize,
            };
        }
