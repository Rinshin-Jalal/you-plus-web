...
export class UserScheduleEngine {
    ...
    supabase; // 🗄️ Database client instance
    constructor(env) {
        this.env = env;
    ...
    async getUsersNeedingCallsNow() {
        try {
            ...
            console.log(`📞 Ready for calls: ${dailyReckoningUsers.length} daily reckoning, ${firstDayUsers.length} first-day`);
            return {
    ...
    async processScheduledCalls() {
        ...
        console.log(`🎬 Processing calls: ${usersNeedingCalls.dailyReckoning.length} daily reckoning, ${usersNeedingCalls.firstDay.length} first-day`);
        // 🥇 STEP 2: Process first-day calls FIRST (highest psychological impact)
        const firstDayResults = await this.batchProcessCalls(usersNeedingCalls.firstDay, "daily_reckoning" // 🚀 Simplified: first-day calls use same daily_reckoning mode
        );
        // 🥈 STEP 3: Process daily reckoning calls (single daily call system)
        const dailyReckoningResults = await this.batchProcessCalls(usersNeedingCalls.dailyReckoning, "daily_reckoning" // 🌇 New single call system with identity psychology
        );
        return {
     ...
     * ═══════════════════════════════════════════════════════════════════════════════ */
    async batchProcessCalls(users, callType) {
        const results = [];
        let processed = 0;
        let errors = 0;
        ...
        for (let i = 0; i < users.length; i += BATCH_SIZE) {
            ...
            console.log(`🔄 Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(users.length / BATCH_SIZE)} - ${batch.length} ${callType} calls`);
            // 🚀 Execute this batch in parallel (max 10 concurrent calls)
            const batchPromises = batch.map(async (user) => {
                try {
                    console.log(`📞 Processing ${callType} call: ${user.id} (${user.timezone})`);
                    ...
                    if (result.success) {
                        ...
                        console.log(`❌ ${callType} call failed: ${user.id} - ${result.error}`);
                    }
                    return {
                        userId: user.id,
                        callType,
                    ...
                    console.error(`💥 Critical error processing ${callType} call for ${user.id}:`, error);
                    return {
                        userId: user.id,
                        callType,
                        success: false,
     ...
     */
    calculateNextCallTime(user, currentTime) {
        try {
            const userLocalTime = new Date(currentTime.toLocaleString("en-US", { timeZone: user.timezone }));
            ...
            console.error(`Error calculating next call time for user ${user.id}:`, error);
            return null;
        }
    }
}
