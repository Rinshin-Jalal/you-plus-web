...
import { checkCallExists } from "@/features/core/utils/database";
 ...
 */
export async function processUserCall(user, callType, env) {
    try {
        const { id: userId, push_token } = user;
        if (!push_token) {
            const errorMessage = `User ${userId} does not have a push token. Cannot initiate call.`;
        ...
        if (callExists) {
            const errorMessage = `Call of type ${callType} already exists for user ${userId} today.`;
        ...
        const pushSent = await sendVoipPushNotification(push_token, {
            userId,
            callType,
        ...
        const errorMessage = `❌ processUserCall failed: ${error instanceof Error ? error.message : String(error)}`;
        console.error(errorMessage);
        return { success: false, error: errorMessage };
    }
}
