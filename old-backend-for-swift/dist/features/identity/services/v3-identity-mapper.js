...
export class V3IdentityMapper {
     ...
     */
    findResponseByDbField(fieldName) {
     ...
     */
    async uploadVoiceRecording(response, userId, filePrefix) {
        ...
        if (typeof response.value === 'string' && response.value.startsWith('data:audio/')) {
            ...
            const uploadResult = await uploadAudioToR2(this.env, audioBuffer, fileName, 'audio/m4a');
     ...
     */
    async extractCoreFields(userId, userName) {
        ...
        const chosen_path = (motivationLevel && motivationLevel >= 7) ? "hopeful" : "doubtful";
        return {
            name,
            daily_commitment,
            chosen_path,
    ...
    async extractVoiceUrls(userId) {
        ...
        if (voiceCommitmentResponse) {
            voiceUrls.commitment_audio_url = await this.uploadVoiceRecording(voiceCommitmentResponse, userId, 'commitment');
        }
        ...
        if (fearVersionResponse) {
            voiceUrls.cost_of_quitting_audio_url = await this.uploadVoiceRecording(fearVersionResponse, userId, 'cost_of_quitting');
        }
        ...
        if (identityGoalResponse) {
            voiceUrls.why_it_matters_audio_url = await this.uploadVoiceRecording(identityGoalResponse, userId, 'why_it_matters');
        }
        return voiceUrls;
    }
    /**
     ...
     */
    async extractIdentity(userId, userName) {
        try {
 ...
 */
export async function extractAndSaveV3Identity(userId, userName, responses, env) {
