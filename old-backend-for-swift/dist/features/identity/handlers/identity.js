 ...
 */
export const getCurrentIdentity = async (c) => {
    ...
    try {
        ...
        const answeredCalls = callStats?.filter((call) => call.duration_sec > 0).length || 0;
 ...
 */
export const updateIdentity = async (c) => {
 ...
 */
export const updateIdentityStatus = async (c) => {
    const userId = getAuthenticatedUserId(c);
    const { currentStreakDays, totalCallsCompleted, lastCallAt, } = await c.req.json();
    const env = c.env;
    const supabase = createSupabaseClient(env);
    try {
 ...
 */
export const getIdentityStats = async (c) => {
    const userId = c.req.param("userId");
    const authenticatedUserId = getAuthenticatedUserId(c);
    if (userId !== authenticatedUserId) {
        return c.json({ error: "Access denied" }, 403);
    ...
    try {
        ...
        const keptPromises = promises?.filter((p) => p.status === "kept").length || 0;
        const brokenPromises = promises?.filter((p) => p.status === "broken").length || 0;
        const successRate = totalPromises > 0 ? Math.round((keptPromises / totalPromises) * 100) : 0;
        const totalCalls = calls?.length || 0;
        const answeredCalls = calls?.filter((c) => c.duration_sec > 0).length || 0;
        const callAnswerRate = totalCalls > 0 ? Math.round((answeredCalls / totalCalls) * 100) : 0;
        ...
        const daysActive = identity
            ? Math.floor((new Date().getTime() - new Date(identity.created_at).getTime()) /
                (1000 * 60 * 60 * 24))
            : 0;
