...
export const requireActiveSubscription = async (c, next) => {
    ...
    try {
        ...
        const subscriptionInfo = await revenueCat.hasActiveSubscription(revenueCatUserId);
        console.log(`📊 RevenueCat subscription result:`, JSON.stringify(subscriptionInfo, null, 2));
        ...
        console.log(`✅ Authenticated request with ACTIVE REVENUECAT subscription for user: ${user.id}`, `Entitlement: ${subscriptionInfo.entitlement}`, `Trial: ${subscriptionInfo.isTrial ? "Yes" : "No"}`);
        return await next();
    }
    catch (error) {
 ...
 */
export const getAuthenticatedUserId = (c) => {
    const userId = c.get("userId");
    if (!userId) {
        throw new Error("User ID not found in context. Ensure requireAuth middleware is used.");
    }
    return userId;
};
/**
 ...
 */
export const requireGuestOrUser = async (c, next) => {
    const authHeader = c.req.header("Authorization");
